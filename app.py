...
st.title("🚢 Indian Ports Integrated Maritime & Hinterland Logistics Platform")
st.markdown("本系統全面整合 **Kaggle 印度 12 大官方主要港口歷史營運資料集** 與 **即時氣象 API**，建構海運塞船預測與陸運卡車調度之一體化決策中心。")
...
with st.sidebar:
    ...
    st.caption("Dataset: Major Ports Traffic & Logistics Time-Series")
    st.markdown("---")
    st.markdown("### 交通流量分析小組")

# 3. 核心大數據資料庫（海運與陸運座標全面綁定）
port_kaggle_database = {
    "Mumbai (孟買港)": {
        ...
        "truck_lat": [18.95, 19.07, 22.71, 28.61], "truck_lon": [72.82, 72.87, 75.85, 77.20], "cities": ["Mumbai Port", "Navi Hub", "Indore Station", "Delhi Terminal"]
    },
    "Chennai (清奈港)": {
        ...
        "truck_lat": [13.09, 12.97, 17.38, 22.57], "truck_lon": [80.30, 77.59, 78.48, 88.36], "cities": ["Chennai Port", "Bangalore Hub", "Hyderabad Hub", "Kolkata Hub"]
    },
    "Kolkata (加爾各答港)": {
        ...
        "truck_lat": [22.54, 23.34, 25.31, 28.61], "truck_lon": [88.31, 85.30, 82.97, 77.20], "cities": ["Kolkata Port", "Ranchi Hub", "Varanasi Station", "Delhi Terminal"]
    },
    "Kandla / Deendayal (坎德拉港)": {
        ...
        "truck_lat": [23.01, 23.02, 26.91, 28.61], "truck_lon": [70.22, 72.57, 75.78, 77.20], "cities": ["Kandla Port", "Ahmedabad Hub", "Jaipur Hub", "Delhi Terminal"]
    },
    "JNPT / Navi Mumbai (新孟買港)": {
        ...
        "truck_lat": [18.95, 19.99, 21.14, 22.71], "truck_lon": [72.94, 73.78, 79.08, 75.85], "cities": ["JNPT Port", "Nashik Hub", "Nagpur Hub", "Indore Station"]
    },
    "Mormugao (莫爾穆加奧港)": {
        ...
        "truck_lat": [15.41, 15.36, 12.97, 9.96], "truck_lon": [73.80, 75.12, 77.59, 76.26], "cities": ["Mormugao Port", "Hubli Station", "Bangalore Hub", "Cochin Hub"]
    },
    "New Mangalore (新芒格洛爾港)": {
        ...
        "truck_lat": [12.93, 12.97, 13.09, 17.38], "truck_lon": [74.81, 77.59, 80.30, 78.48], "cities": ["New Mangalore Port", "Bangalore Hub", "Chennai Hub", "Hyderabad Hub"]
    },
    "Cochin (柯枝港)": {
        ...
        "truck_lat": [9.96, 11.01, 12.97, 13.09], "truck_lon": [76.26, 76.95, 77.59, 80.30], "cities": ["Cochin Port", "Coimbatore Hub", "Bangalore Hub", "Chennai Hub"]
    },
    "Tuticorin / V.O.C. (圖蒂科林港)": {
        ...
        "truck_lat": [8.75, 9.92, 11.01, 13.09], "truck_lon": [78.19, 78.12, 76.95, 80.30], "cities": ["Tuticorin Port", "Madurai Station", "Coimbatore Hub", "Chennai Hub"]
    },
    "Visakhapatnam (維沙卡帕特南港)": {
        ...
        "truck_lat": [17.69, 16.50, 17.38, 22.57], "truck_lon": [83.29, 80.64, 78.48, 88.36], "cities": ["Vizag Port", "Vijayawada Hub", "Hyderabad Hub", "Kolkata Hub"]
    },
    "Paradip (帕拉迪普港)": {
        ...
        "truck_lat": [20.26, 20.46, 22.57, 25.31], "truck_lon": [86.67, 85.87, 88.36, 82.97], "cities": ["Paradip Port", "Cuttack Station", "Kolkata Hub", "Varanasi Station"]
    },
    "Ennore / Kamarajar (恩諾爾港)": {
        ...
        "truck_lat": [13.25, 13.09, 12.97, 17.38], "truck_lon": [80.34, 80.30, 77.59, 78.48], "cities": ["Ennore Port", "Chennai Hub", "Bangalore Hub", "Hyderabad Hub"]
    }
}
...
# 5. 設計頂部智慧分頁頁籤 (Tabs)
tab1, tab2 = st.tabs(["⚓ Maritime Core (海運大船進港預測)", "🚛 Hinterland Logistics (陸運卡車調度最佳化)"])
...
with tab1:
    ...
    if is_congested:
        st.error(f"**【海運警報】🚨 CRITICAL CONGESTION RISK** \n\n 目前即時風速達 {current_wind} km/h，結合 Kaggle 歷史數據顯示該港口屬於易擁擠體質。預估極端天氣將導致港區潛在延誤經濟損失高達 **${sea_loss:,.0f} USD**！")
    else:
        st.success(f"**【海運狀態】✅ NORMAL OPERATIONAL STATUS** \n\n 當前風速安全。交叉比對 Kaggle 歷史平均清關時間（{port_info['avg_wait_hours']} 小時），日常營運正常，機會成本維持在 **${sea_loss:,.0f} USD**。")
...
with tab2:
    ...
    truck_loss = truck_count * (wait_after / 60) * (cost_per_hour / 10) # 模擬陸運規模成本
    
    st.info(f"💡 **陸運優化成效：智慧預約排程已成功為卡車車隊節省了 {time_saved:.1f} 分鐘 的塞車時間，當前調度陸運成本控制在 ${truck_loss:,.0f} USD。**")
    
    col_t1, col_t2 = st.columns(2)
    with col_t1:
        st.metric(label="傳統隨機進港 - 平均回堵時間", value=f"{wait_before:.1f} 分鐘", delta="陸運交通易癱瘓", delta_color="inverse")
    with col_t2:
        st.metric(label="時間窗排程 - 優化後回堵時間", value=f"{wait_after:.1f} 分鐘", delta="效率大幅提升 66%")
